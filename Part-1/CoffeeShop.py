...
class CoffeeShop:
    ...
    def get_all_speciality_staff_name(self):
        """Returns a list of all baristas names who have a coffee making
        speciality."""
        return self.all_speciality_staff_name

    def get_all_normal_staff_name(self):
        """Returns a list of all baristas names who do not have a coffee making
        speciality."""
    ...
    def print_header(self):
        ...
        print("=============================="
              f" SIMULATING MONTH {self.simulation_month} "
              "==============================")
    ...
    def maximise_income(self):
        """Uses bubble sort to sort coffeetypes dictionary based on largest
        price per unit time of barista so that coffee demand is attended
        accordingly to maximize income."""
        ...
        for i in range(n):
            ...
            for j in range(0, n-i-1):
                ...
                j_0 = ((list_of_coffeetypes[j].get_sell_price())
                       /(list_of_coffeetypes[j].get_prep_time()))

                j_1 = ((list_of_coffeetypes[j+1].get_sell_price())
                       /(list_of_coffeetypes[j+1].get_prep_time()))

                if j_0 < j_1:

                    list_of_coffeetypes[j], list_of_coffeetypes[j+1] = (
                        list_of_coffeetypes[j+1], list_of_coffeetypes[j])
    ...
    def add_barista(self):
        ...
        while name_exists is True:
            ...
            if (name==""
                    or any(char in characters for char in name)
                    or any(char in exception for char in name)):

                print("Please enter a name with no numbers and no "
                      "special characters!")
                ...
                if name in self.chosen_baristas:
                    ...
                    print("Barista name already exists, please enter"
                          " a unique name!")
    ...
    def remove_barista(self):
        """Removes barista from the chosen_baristas dictionary and
        all_speciality_staff_name list."""
        ...
        while name_exists is False:
            ...
            if name!="":
                ...
                if name in self.chosen_baristas:
                    ...
                    print(f"Removed {name}, hourly rate "
                          f"= £{self.chosen_baristas[name].get_rate_per_hour():.2f}"
                          f" in month {self.simulation_month}")
                    ...
                    print("Please enter a valid barista name. "
                          "Here are the current baristas: ")
    ...
    def select_speciality(self, name, barista):
        """
        Prompts user to enter barista's coffee making speciality (if any) from
        a list of coffees served at the shop.
        ...
        while speciality_valid_response is False:
            ...
            while loop_bool is False:
                ...
                print("Does this barista have a speciality in "
                      "any one of the above coffee types?")
            ...
            if speciality_exists.lower()=="y" or speciality_exists.lower()=="yes":
                ...
                if speciality in list(self.coffeetypes.keys()):
                    ...
                    # assigns to speciality within barista object
                    ...
                    print("Please enter a valid type of coffee! "
                          "(Note: response is case sensitve)")
        ...
        print(f"Added {name}, hourly rate "
              f"= £{self.chosen_baristas[name].get_rate_per_hour():.2f}"
              f" in month {self.simulation_month}")
    ...
    def assign_demand(self, rem, coffeetype, demand:int, barista_list:list, speciality:bool):
        ...
        barista_list : list
            list of baristas who are to be assigned the time to fulfil the
            demand
        speciality : bool
            a boolean to indicate whether the list of barista contains baristas
            with any specialities any of the coffee types
        ...
        for name in barista_list:
            ...
            if self.chosen_baristas[name].get_hrs_worked()!=80:
                ...
                avl = (Fraction(80) -
                       Fraction(self.chosen_baristas[name].get_hrs_worked()))

                # if available (avl) hrs for each barista is greater than hrs
                # demand for serving type of coffee then all the hrs are assigned
                # to one barista, else the remainder (rem) number of hrs is
                # assigned to the next barista in line
                if rem >0:
    ...
    def print_status(self):
        """Prints out the shop name, cash status, quantity of each ingredient
        available and hired baristas."""
        ...
        for ingredient in list(self.ingredients.values()):
            ...
            print(f"\t\t {ingredient.get_name()}, remaining "
                  f"{ingredient.get_leftover_quantity():.2f}{units}"
                  f" (capacity = {ingredient.get_capacity()}{units})")
        ...
        for barista in list(self.chosen_baristas.values()):
            ...
            print(f"\t\tBarista {barista.get_name()}, "
                  f"hourly rate = £{barista.get_rate_per_hour():.2f}")


    def collect_coffee_income(self):
        """Calculates the amount earned from quantities sold for each type of
        coffee."""
    ...
    def pay_monthly_rent(self):
        """Returns a boolean to indicate if there sufficient cash to pay the
        monthly rent and if there is then rent is deducted from the cash held
        by the shop."""
        ...
        if (self.current_cash - self.fixed_monthly_rent) < 0:
            ...
            print(f"Require £{self.fixed_monthly_rent:.2f},"
                  f" but only have £{self.current_cash:.2f}!")
    ...
    def pay_baristas(self):
        """Returns a boolean to indicate if there sufficient cash to pay all
        the baristas and if there is then salaries are deducted from the cash
        held by the shop."""
        ...
        for barista in list(self.chosen_baristas.values()):
            ...
            if (self.current_cash - barista_salary)<0:
                ...
                # informs user how much is required vs current cash that shop has
                print(f"Require £{(barista_salary):.2f}, "
                      f"but only have £{self.current_cash:.2f}!")
                ...
                print(f"\tPaid {barista.get_name()}, hourly rate "
                      f"= £{barista.get_rate_per_hour():.2f}, "
                      f"amount £{(barista_salary):.2f}")

        return bankrupt

    def pay_pantry_costs(self):
        """Returns a boolean to indicate if there sufficient cash to pay all
        the pantry costs and if there is then costs are deducted from the cash
        held by the shop."""
        ...
        for ingredient in list(self.ingredients.values()):
            ...
            if (self.current_cash - pantry_cost) <0:
                ...
                print("BANKRUPT: Insufficient cash to pay pantry "
                      f"costs for {ingredient.get_name()}!")
    ...
    def restock_ingredients(self):
        """Returns a boolean to indicate if there sufficient cash to restock
        all the ingredients and if there is then costs are deducted from the
        cash held by the shop."""
        ...
        while valid_response is False:
            ...
            supplier_choose = input("Please select a supplier from the above "
                                    "list to restock ingredients: ").strip().title()
        ...
        # looping through each ingredient to get the updated value of cash and
        # reset its quantity
        for ingredient in list(self.ingredients.values()):

            if (self.current_cash -
                ingredient.get_restock_cost(self.suppliers[supplier_choose])) < 0:

                print (f"BANKRUPT: Insufficient cash to restock {ingredient.get_name()}!")

                # informs user how much is required vs current cash that shop has
                print ("Require "
                       f"£{(ingredient.get_restock_cost(self.suppliers[supplier_choose])):.2f}, "
                       f"but only have £{self.current_cash:.2f}!")
                ...
                self.current_cash -= ingredient.get_restock_cost(
                    self.suppliers[supplier_choose])
